...
function renderMarkdown(raw: string) {
  ...
  const renderInline = (text: string): React.ReactNode => {
    ...
    return parts.map((p, i) => {
      ...
      if (p.startsWith('`') && p.endsWith('`'))
        return <code key={i} className="px-1 rounded text-xs font-mono" style={{ background: 'rgba(167,139,250,0.08)', color: '#a78bfa' }}>{p.slice(1, -1)}</code>;
      return p;
    });
  };
  ...
  while (i < lines.length) {
    ...
    if (/^#{2,3}\s+/.test(line)) {
      ...
      nodes.push(
        <div key={key++} className="text-[10px] font-black tracking-widest mt-4 mb-1.5 first:mt-0" style={{ color: '#a78bfa' }}>
          {text.toUpperCase()}
        </div>
      );
      i++; continue;
    ...
    if (/^#\s+/.test(line)) {
      ...
      nodes.push(
        <div key={key++} className="text-xs font-black tracking-widest mt-4 mb-2 first:mt-0" style={{ color: '#a78bfa' }}>
          {text.toUpperCase()}
        </div>
      );
      i++; continue;
        ...
        <ul key={key++} className="flex flex-col gap-1 my-1.5 ml-1">
          {items.map((item, j) => (
            <li key={j} className="flex gap-2 text-sm leading-relaxed" style={{ color: '#94a3b8' }}>
              <span className="mt-[6px] w-1 h-1 rounded-full flex-shrink-0" style={{ background: '#35354d' }} />
    ...
    // Horizontal rule
    if (/^---+$/.test(line)) {
      nodes.push(<div key={key++} className="my-3" style={{ height: 1, background: '#1e1e32' }} />);
      i++; continue;
    }
...
function MinuteKpiCard({ label, sublabel, value, thresholds }: { label: string; sublabel: string; value: number | null; thresholds: [number, number] }) {
  const color = value === null ? '#35354d' : value <= thresholds[0] ? '#22c55e' : value <= thresholds[1] ? '#f97316' : '#ef4444';
  const grade = value === null ? '' : value <= thresholds[0] ? 'ÓPTIMO' : value <= thresholds[1] ? 'ACEPTABLE' : 'CRÍTICO';
  return (
    <div className="flex items-center gap-4 flex-1">
      <div>
        <div className="text-[10px] font-black tracking-widest mb-0.5" style={{ color: '#525270' }}>{label}</div>
      ...
      <div className="text-[11px] leading-relaxed max-w-[180px]" style={{ color: '#35354d' }}>{sublabel}</div>
    </div>
  );
}
...
function KpiBlock({ value, label }: { value: number; label: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3 flex-1">
      ...
      <span className="text-[10px] font-bold tracking-widest" style={{ color: '#525270' }}>{label}</span>
    </div>
  );
}
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  h1 { font-size: 22px; font-weight: 900; color: #0e0e18; margin-bottom: 2px; }
  .subtitle { color: #64648a; font-size: 11px; margin-bottom: 28px; }
  ...
  .duration-value { font-size: 28px; font-weight: 900; color: #7c3aed; font-family: monospace; }
  ...
  .sap-label { font-size: 10px; font-weight: 600; color: #35354d; margin-top: 2px; }
  .narrative { border: 1px solid #e2e8f0; border-radius: 10px; padding: 18px; line-height: 1.7; color: #35354d; white-space: pre-wrap; }
  ...
  .pending-id { font-size: 12px; font-weight: 900; color: #0e0e18; }
  .pending-meta { font-size: 10px; color: #94a3b8; margin-top: 1px; }
  .pending-action { font-size: 11px; color: #525270; margin-top: 5px; line-height: 1.5; }
...
<body>
  ...
  <div class="stat-grid">
    ...
    <div class="stat-box">
      <div><span class="stat-value" style="color:#7c3aed">${attended}</span><span class="stat-unit">/ ${faults.length}</span></div>
  ...
  <div class="sap-grid">
    ...
    <div class="sap-box"><div class="sap-system">SAP Field Service Management</div><div class="sap-value" style="color:#7c3aed">${actionMessages.filter(a => a.system === 'SAP Field Service Management' && a.msg.includes('Orden de trabajo')).length}</div><div class="sap-label">Órdenes de trabajo creadas</div></div>
    ...
    <div class="sap-box"><div class="sap-system">SAP S/4HANA Asset Management</div><div class="sap-value" style="color:#7c3aed">${faults.length}</div><div class="sap-label">Activos analizados en S/4HANA</div></div>
  </div>
  ...
  ${pendingRows.map(fault => {
    ...
    return `<div class="pending-row" style="border-color:${col}33">
      ...
      <div style="flex:1">
        <div class="pending-id">${fault.id} <span style="font-weight:400;color:#64648a;font-size:11px">${fault.zone} · ${fault.type === 'transformer' ? 'Transformador' : fault.type === 'cable' ? 'Cable' : 'Conmutable'} · ${fault.affectedClients.toLocaleString('es-ES')} clientes${fault.criticalSite ? ` · ${fault.criticalSite}` : ''}</span></div>
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background: 'linear-gradient(160deg,#0e0e1c 0%,#0a0a10 100%)',
          border: '1px solid #1e1e32',
          boxShadow: '0 0 100px rgba(167,139,250,0.07), 0 40px 80px rgba(0,0,0,0.8)',
          transform: vis ? 'scale(1)' : 'scale(0.95)',
          transition: 'transform 0.4s cubic-bezier(0.4,0,0.2,1)',
        }}
      >
        {/* ── Header ── */}
        <div className="flex items-center gap-3 px-6 py-4 flex-shrink-0" style={{ borderBottom: '1px solid #1e1e32' }}>
          <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
          <span className="text-xs font-black tracking-widest" style={{ color: '#a78bfa' }}>RESUMEN EJECUTIVO</span>
          <span className="text-xs font-mono" style={{ color: '#1a1a30' }}>·</span>
          <span className="text-xs font-mono" style={{ color: '#35354d' }}>Ciclo completado · {elapsedLabel}</span>
          ...
          <button
            ...
            style={{ background: 'rgba(255,255,255,0.04)', color: '#525270', border: '1px solid #1e1e32', cursor: 'pointer' }}
          >✕</button>
        </div>
        ...
        <div className="p-6 flex flex-col gap-5">
          ...
          {/* ── KPIs ── */}
          <div className="rounded-2xl px-8 py-6 flex flex-col gap-5" style={{ background: 'rgba(10,18,35,0.6)', border: '1px solid #1e1e32' }}>
            <div className="flex items-center gap-8">
              <KpiBlock value={kpi.sla ?? 0} label="SLA" />
              <div style={{ width: 1, height: 100, background: '#1e1e32', flexShrink: 0 }} />
              <KpiBlock value={kpi.safety ?? 0} label="SEGURIDAD" />
              <div style={{ width: 1, height: 100, background: '#1e1e32', flexShrink: 0 }} />
              <KpiBlock value={kpi.efficiency ?? 0} label="EFICIENCIA OPERATIVA" />
              <div style={{ width: 1, height: 100, background: '#1e1e32', flexShrink: 0 }} />
              <div className="flex flex-col items-center gap-1 flex-shrink-0">
                <div className="text-4xl font-black font-mono" style={{ color: '#a78bfa' }}>{elapsedLabel}</div>
                <div className="text-[10px] font-bold tracking-widest mt-1" style={{ color: '#525270' }}>DURACIÓN CICLO</div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-10">
              ...
              <div style={{ width: 1, height: 48, background: '#1e1e32', flexShrink: 0 }} />
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              ...
              <div key={s.label} className="rounded-xl p-4" style={{ background: 'rgba(10,18,35,0.6)', border: '1px solid #1e1e32' }}>
                <div className="flex items-baseline gap-1 mb-1">
                  <span className="text-3xl font-black" style={{ color: s.color }}>{s.value}</span>
                  {s.unit && <span className="text-sm font-bold" style={{ color: '#35354d' }}>{s.unit}</span>}
                </div>
                <div className="text-xs font-bold text-white mb-0.5">{s.label}</div>
                <div className="text-[11px]" style={{ color: '#35354d' }}>{s.sub}</div>
              </div>
            ))}
          </div>
          ...
          <div>
            <div className="text-[10px] font-bold tracking-widest mb-3" style={{ color: '#1a1a30' }}>INTEGRACIÓN SAP</div>
            <div className="grid grid-cols-3 gap-3">
              ...
              <SapKpiCard system="SAP S/4HANA Asset Management" value={s4Assets} label="Activos analizados en S/4HANA" color="#a78bfa" />
            </div>
          </div>

          {/* ── Status Update executive summary ── */}
          <div>
            <div className="text-[10px] font-bold tracking-widest mb-3" style={{ color: '#1a1a30' }}>ANÁLISIS STATUS UPDATE</div>
            <div className="rounded-xl px-6 py-5" style={{ background: 'rgba(10,18,35,0.6)', border: '1px solid #1e1e32' }}>
              {orchRaw ? (
                <div>{renderMarkdown(orchRaw)}</div>
              ) : (
                <p className="text-sm italic" style={{ color: '#35354d' }}>Resumen del orquestador no disponible.</p>
              )}
            </div>
          </div>
          ...
          {pendingSorted.length > 0 && (
            <div>
              <div className="flex items-center gap-3 mb-3">
                <div className="text-[10px] font-bold tracking-widest" style={{ color: '#1a1a30' }}>ACCIONES PENDIENTES</div>
                    ...
                    <div key={fault.id} className="rounded-xl p-4 flex gap-4" style={{ background: 'rgba(10,18,35,0.6)', border: `1px solid rgba(${hexToRgb(uColor)},0.25)` }}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          ...
                          <span className="text-xs font-mono" style={{ color: '#525270' }}>{fault.zone}</span>
                          <span className="text-xs font-mono" style={{ color: '#35354d' }}>
                        ...
                        <p className="text-xs leading-relaxed" style={{ color: '#64648a' }}>
        ...
        {/* ── Footer ── */}
        <div className="flex items-center justify-between px-6 py-4 flex-shrink-0 mt-auto" style={{ borderTop: '1px solid #11111d' }}>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-mono" style={{ color: '#1a1a30' }}>Storm Response Commander · Iberdrola Girona</span>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={downloadReport}
              className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-bold"
              style={{ background: 'rgba(167,139,250,0.05)', color: '#525270', border: '1px solid #1e1e32', cursor: 'pointer' }}
            >
            ...
            <button
              ...
              style={{ background: 'rgba(167,139,250,0.07)', color: '#a78bfa', border: '1px solid rgba(167,139,250,0.2)', cursor: 'pointer' }}
            >
